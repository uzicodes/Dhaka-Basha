  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status === "complete") {
        ...
        console.log(result);
        setClerkError("সাইন আপ অসম্পূর্ণ। যাচাইকরণ প্রয়োজন।");
      }
    } catch (err: any) {
      setClerkError(err.errors?.[0]?.longMessage || "সাইন আপ ব্যর্থ হয়েছে। দয়া করে আবার চেষ্টা করুন।");
    }
  };
    ...
    <main className="grow flex flex-col items-center justify-center px-4 bg-slate-50 pt-32 pb-12">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-1 bg-white p-5 w-full max-w-112.5 rounded-[20px] shadow-sm border-2 border-[#2d79f3]">
        ...
        <button
          disabled={!isLoaded}
          type="submit"
          className="mt-6 mb-2 bg-[#151717] text-white text-[14px] font-medium rounded-[10px] h-11 w-50 mx-auto cursor-pointer hover:bg-black hover:text-green-500 hover:shadow-lg hover:scale-105 transition-all duration-200 disabled:opacity-50"
        >
          সাইন আপ করুন
        </button>
        ...
        <button type="button" className="mt-2 w-30 h-11 mx-auto rounded-[10px] flex justify-center items-center font-medium text-[14px] text-black gap-2 border border-[#2d79f3] bg-white cursor-pointer transition-all duration-200 hover:bg-green-200 hover:shadow-lg hover:scale-105">
